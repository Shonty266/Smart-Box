...
def send_data_to_server(data):
    url = 'http://localhost:8080/endpointforreceiver'  # Replace with your server's URL
    payload = {'data': data}
    try:
...
def run():
    ...
    while True:
        ...
        if decodedObjects:
            for obj in decodedObjects:
                ...
                # Check if the current data is different or if enough time has passed
    ...
    # Display "Ready for another run" before closing
    cv2.putText(frame, "Ready for another run!", (50, 100), font, 2, (0, 255, 0), 3)
    cv2.imshow("Please Scan the QR Code", frame)
